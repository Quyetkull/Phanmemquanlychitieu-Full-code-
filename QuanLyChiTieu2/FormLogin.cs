...
namespace QuanLyChiTieu2
{
    public partial class FormLogin : Form
    {
        ...
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-MQ3E0HG\SQLEXPRESS;Initial Catalog=QLCT;Integrated Security=True");
            try
            {
                conn.Open(); // mở kết nối database
                string tk = textBoxUserName.Text;
                string mk = textBoxPassWord.Text;
                string sql = " select * from Tk where UserName = '" + tk + "'and PassWord ='" + mk + "' ";
